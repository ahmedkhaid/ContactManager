...
namespace CRUDExample.Filters.ActionFilter
{
    public class PersonCreateAndEditActionFilter : IAsyncActionFilter
    {
        ...
        public PersonCreateAndEditActionFilter(ICountriesGetterService countriesService)
        {
        ...
        public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
        {
            if (context.Controller is PersonsController personsController)
            {
                if (!personsController.ModelState.IsValid)
                {
                    ...
                    personsController.ViewBag.Errors=personsController.ModelState.Values.SelectMany(e => e.Errors).Select(e => e.ErrorMessage);
